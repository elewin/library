      ...
      .then(function(library){
        ...
        if (foundBookIndex === -1){
          ...
          library.save().then(function(){ //save the updated book
            ...
            //if we updated the status of the book:
            if (property === 'status'){
            ...
            return foundBook.save();
              ...
              console.log('libraryCtrl.editBookInLibrary: ', err);
  ...
  addBookToLibrary: function(req, res){
    ...
    var addBook = function(bookId){
      ...
      findDuplicate(libraryId, bookId).then(function(duplicate){
        if(!duplicate){
          //make sure we're not channging a library we're not supposed to
          if (req.user && (req.user.library === libraryId || req.user.roles.indexOf('admin') >=0)){
            Library.findById(libraryId).exec() //find this library
            .then(function(library){
              var books = library.books; //after we've found it, get the array of books in the library that we will be adding to
              books.push(bookObj); //add the new book to the array of books
              return library.save(); //save the library with our new book in it
            }).then(function(){
              return Book.findById(bookId).exec(); //find this book by its bookId
            }).then(function(thisBook){
              thisBook.numOwners++; //increment the number of owners of this book
              return thisBook.save(); //save this book
            }).then(function(){
              return res.status(201).end();
            }).catch(function(err){ //uh-oh
              console.log('libraryCtrl.addBookToLibrary/addBook: ', err);
  ...
  removeBookFromLibrary: function(req, res){
    ...
    if (req.user && (req.user.library === req.params.id || req.user.roles.indexOf('admin') >=0)){
      Library.findById(req.params.id).exec().then(function(library){
        ...
        if(bookToRemoveIndex === -1){ //if no match is found (this should only happen if something is broken)
          console.log('Book', bookToDelete, 'not found in library', req.params.id);
          return res.status(404).end();
        }

        //get their stats so we can adjust the stat counters for this book:
        own = books[bookToRemoveIndex].book.own;
        status = books[bookToRemoveIndex].book.status;
        rating = books[bookToRemoveIndex].book.rating;

        books.splice(bookToRemoveIndex, 1); //remove the book at the index where it was found
        return library.save(); //save the result
      }).then(function(theLibrary){
        return Book.findById(bookToDelete).exec(); //now lets grab the book from the main book collection so we can update it
